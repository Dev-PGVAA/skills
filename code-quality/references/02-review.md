# Part 2 — Review (defect-first code review)
...
- `P0`: universal release blocker or critical failure. **Security-sensitive paths are P0 when the change touches auth, secrets, session, authorization, injection surfaces, or secret logging — even if the rest of the diff is a feature.** Route deeper threat work to the `security` skill part `audit`.
