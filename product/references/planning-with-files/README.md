...
> **Note:** this folder is embedded documentation of the former standalone planning-with-files skill, now part of the `product` skill (part 4, file-planning). The hooks and slash commands described below (`/plan-attest`, `/plan-goal`, `/plan-loop`, PreCompact) belong to that standalone installation and are NOT active here. The scripts and templates in this folder work standalone — run them by path from this folder.
...
```bash
# Linux/macOS (path relative to this planning-with-files folder)
$(command -v python3 || command -v python) scripts/session-catchup.py "$(pwd)"
```

```powershell
# Windows PowerShell (path relative to this planning-with-files folder)
...
- **Templates** are in `templates/` inside this folder
- **Your planning files** go in **your project directory**

| Location | What Goes There |
|----------|-----------------|
| This folder (`planning-with-files/`) | Templates, scripts, reference docs |
...
```bash
# user-wide (run from the planning-with-files folder)
cp templates/loop.md ~/.claude/loop.md
